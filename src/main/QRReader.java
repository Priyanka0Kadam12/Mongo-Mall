...
public class QRReader extends JPanel implements java.beans.PropertyChangeListener, ActionListener {
	...
	DSFilterInfo[][] dsi;
	private ItemWindow w;
	private AddUpdateProduct aup;
	...
	public QRReader(ItemWindow w,AddUpdateProduct aup,UpdateProduct up)
	{
		this.w=w;
		this.aup=aup;
		this.up=up;
	...
	public void actionPerformed(ActionEvent e) {
		...
		for(ZBar.Symbol s:ls){
			...
			code=s.data;
			if(w!=null){
			w.setBarcodeText(code);
			}else{
				if(aup!=null){
					aup.setproBarcodeText(code);
			}else
			up.setTfBarcode(code);
			}
